...
static void setup_numa_nodes(void) {
    ...
    numa_nodes[0].used_pages = numa_nodes[0].total_pages - numa_nodes[0].free_pages;
    numa_nodes[1].used_pages = numa_nodes[1].total_pages - numa_nodes[1].free_pages;
...
paddr_t pmm_alloc_pages(uint32_t count, uint8_t node) {
    if (count == 0) return 0;
    if (count == 1) return pmm_alloc_page(node);
